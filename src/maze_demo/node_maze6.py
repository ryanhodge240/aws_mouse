...
class Micromouse_Node(object):
    ...
    def runMaze(self):
        ...
        ki = 0
        kd = 0
        self.prevleftTurn = False
        self.leftTurn = False
        rate = rospy.Rate(10)
        long_error = rospy.Publisher('/long_error',Float32, queue_size=1)
        zero_error = rospy.Publisher('/zero_error',Float32, queue_size=1)

        self.file1 = open('myfile.txt', 'w')
	t0 = rospy.Time.now().to_sec()

        while not rospy.is_shutdown():
	    if (self.laser_sensors['l'] >0):
		t1 = rospy.Time.now().to_sec()
            	self.file1.writelines("{}, {},{},{}\n".format(t1-t0, self.laser_sensors['l'],  self.laser_sensors['f'], self.laser_sensors['r']))
	    	self.file1.flush()
...
#            self.turnleft()
    ...
    def follow_right_wall(self):
        global kp,ki,kd
        global long_error, zero_error, previous_error, error, integ
        kp =  121
        ki = 0
        kd = 0
        ...
        output = -kp*error - kd*diff - ki*integ 
        msg.linear.x = AC
#        print("output->:{:.3f} and left distance->: {:.3f}, current right distance->: {:.3f}".format(output, self.laser_sensors['l'], self.laser_sensors['r']))
    ...
    def turnleft(self):   
    	...
    	kp=0.5
    	target = 90
    	target_rad = target* math.pi/180 + current_heading   # current heading add 90 degree
    	self.prev_yaw_imu =90
